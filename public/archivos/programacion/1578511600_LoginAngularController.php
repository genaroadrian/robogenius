...
class LoginAngularController extends Controller
{
    ...
    public function validation(Request $request)
    {
        ...
        if ($em <> null) {
            ...
            $email_verified = DB::SELECT("SELECT email_verified_at from users where email = '$email'");
            foreach ($email_verified as $m) {
                $verified = $m->email_verified_at;
            }
            ...
            foreach ($exist as $e) {
                if ($e->num == 1) {
                    ...
                    if(($x == $clave) && ($nombre == $email && $verified == null))
                    {
